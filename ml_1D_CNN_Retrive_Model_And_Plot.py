...
with open ('df_actual_pred-2.pickle', 'rb') as f:
...
df_final = df_final.iloc[warm_up_steps:].reset_index(drop=True)

df_for_testing_X = df_for_testing_X.iloc[warm_up_steps:].reset_index(drop=True)
...
axs[3].plot(df_for_testing_X[0:]["mic1_coh"], c ="blue", label=r'$\rho_{1}$')
axs[3].plot(df_for_testing_X[0:]["mic2_coh"], c ="red",  label=r'$\rho_{2}$')
axs[3].grid()
axs[3].legend(loc= 'upper right')
axs[3].set_ylabel(r'$\rho$')  

axs[4].plot(mic1_abs_test[0:], c ="blue", label=r'D1$_A$')
axs[4].plot(mic2_abs_test[0:], c ="red", label=r'D2$_A$')
axs[4].grid()
axs[4].legend(loc= 'upper right')
axs[4].set_ylabel(r'$|x|$')  
axs[4].set_xlabel('$x$ (locus steps)')
plt.savefig('1d_cnn.pdf')
