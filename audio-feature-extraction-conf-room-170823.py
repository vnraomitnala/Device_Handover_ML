# -*- coding: utf-8 -*-
"""
Created on Thu Aug 17 18:51:21 2023
...
directory1 = "dev-clean1-converted-10"
...
conf_room_dim = [10, 7, 2.7] #[7,5.5,2.4] #


confdRoom_mic_locs = [
    [1.0,5.5, 1.9], [7.5,5.5,1.9],  # mic 1  # mic 2
]
...
rooms = []
for i in range(len(confdRoom_mic_locs)):
    room = pra.ShoeBox(conf_room_dim, fs=fs, materials=pra.Material(e_absorption), max_order=max_order)
    
    phi0_radient = 90 * math.pi/180
    
    R = pra.circular_2D_array(confdRoom_mic_locs[i][:2], 2, phi0_radient, 0.05)
    R = np.concatenate((R, np.ones((1, 2)) * confdRoom_mic_locs[i][2]), axis=0)
...
standardRoom_source_locs = [ [ 1.5 +i*0.15,1, 1.4] for i in range(50) ]
...
# =============================================================================
    ...
    for l in range(1):
        ...
        locus = standardRoom_source_locs# locusDataset[l]["locus"] #standardRoom_source_locs #locusDataset["locus"][0]
        travel_time = [ 0.2 +i*0.25 for i in range(50)] #locusDataset[l]["travelTime"]
...
with open ('datasetFinal_linear_locus_dev-clean1-conf_sig1.pickle', 'wb' ) as f:
    pickle.dump(datasetFinal, f) 
    
with open ('travelTimeDataset_random_locus_dev-clean1-conf_sig1.pickle', 'wb' ) as f:
