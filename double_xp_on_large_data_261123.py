...
for i in range(len( distance_dataset_testing)):
    distance = flat_list(get_item(distance_dataset_testing[i],'mic1-distance' ))
    mic1_disc_test.append(distance)
    

for i in range(len(distance_dataset_testing)):
    distance = flat_list(get_item(distance_dataset_testing[i],'mic2-distance' ))
    mic2_disc_test.append(distance)
    
mic1_distance_test =  np.concatenate(mic1_disc_test, axis=0).tolist() #flat_list(get_item(distance_dataset_testing[0], 'mic1-distance'))
mic2_distance_test = np.concatenate(mic2_disc_test, axis=0).tolist() #flat_list(get_item(distance_dataset_testing[0], 'mic2-distance'))
...
axs[0].plot(pred_listMic[0:200],c ="blue", label='S')
axs[0].legend(loc= 'upper left')
axs[0].grid()
axs[0].set_ylabel('$y$ (transition)')

axs[1].plot(listMic[0:200], c ="green", label='NS')
axs[1].legend(loc= 'upper left')
axs[1].grid()  
axs[1].set_xlabel('$x$ (locus steps)') 
...
wtd_mse_smooth = np.mean(wtd_mse)

print("wtd_mse_smooth: ", wtd_mse_smooth)                                                                             #print('wtd_mse_with_smooth: ', round(wtd_mse_mean,2))
