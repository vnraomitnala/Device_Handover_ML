...
test_X =   df_audio_features[test_signal_index][['mic1_coh' ,"mic2_coh", 'mic1_abs_l', "mic1_abs_r", 'mic2_abs_l', "mic2_abs_r" ]] # df_audio_features[1] #
 # df_audio_features[1] #
...
model = Sequential()
# Add Input layer
model.add(keras.layers.Input(shape=(49, 6)))
...
loss = model.evaluate(x_test, y_test, batch_size=no_timesteps, verbose=0)

print("loss: ",  loss)
print( "       ")

with open ('model_10_200_50-2.pickle', 'wb' ) as f:
        pickle.dump(model, f) 
...
with open ('df_actual_pred-2.pickle', 'wb' ) as f:
...
axs[0].plot((df_final[warm_up_steps:]['dist-diff'].reset_index(drop=True)), c ="blue", label='GTruth')
axs[0].grid()
...
for i in range(len(df_final['pred-dist-diff'])):
    if df_final['pred-dist-diff'][i] < 1.5:
        pred_listMic.append('1')
    else:
        pred_listMic.append('2')

axs[1].plot(pred_listMic[warm_up_steps:], c ="red", label='Pred.')
axs[1].grid()
...
axs[2].plot(listMic[warm_up_steps:], c ="orange", label= r'$\max({\rho_1},{\rho_2})$')
...
axs[3].plot(df_for_testing_X[warm_up_steps:]["mic1_coh"].reset_index(drop=True), c ="blue", label=r'$\rho_{1}$')
axs[3].plot(df_for_testing_X[warm_up_steps:]["mic2_coh"].reset_index(drop=True), c ="red", label=r'$\rho_{2}$')
axs[3].grid()
axs[3].legend(loc= 'upper right')
axs[3].set_ylabel(r'$\rho$')  

axs[4].plot(mic1_abs_test[warm_up_steps:], c ="blue", label='D1$_A$')
axs[4].plot(mic2_abs_test[warm_up_steps:], c ="red", label='D2$_A$')
axs[4].grid()
axs[4].legend(loc= 'upper right')
axs[4].set_ylabel(r'$|x|$')  
axs[4].set_xlabel('$x$ (locus steps)') 

plt.savefig('1d_cnn_randum_move.pdf')
