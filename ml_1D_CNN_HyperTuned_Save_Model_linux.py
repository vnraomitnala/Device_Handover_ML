...
with open ('C:/Users/Vijaya/PhD/chapter5/SignalDataset/datasetFinal_randum_locus_dev-clean1-10-200-100.pickle', 'rb') as f:
     audio_features_dataset_10_locus_1 = pickle.load(f)                

with open ('C:/Users/Vijaya/PhD/chapter5/SignalDataset/distanceDataset_randum_locus_dev-clean1-10-200-100.pickle', 'rb') as f:
     distance_dataset_10_locus_1 = pickle.load(f)
...
def build_model(hp):
    ...
    model.add(layers.Conv1D(filters=hp.Int('filters_1', min_value=100, max_value=128, step=32),
                            kernel_size=hp.Int('kernel_1', min_value=100, max_value=200, step=10),
                            activation='relu',
                            strides=1,
                            padding='same',
    ...
    model.add(layers.Conv1D(filters=hp.Int('filters_2', min_value=64, max_value=128, step=32),
                            kernel_size=hp.Int('kernel_2', min_value=100, max_value=200, step=10),
                            activation='relu',
                            strides=1,
                            padding='same',
                            input_shape=(no_timesteps, no_featues)))
...
tuner.search(X_train, y_train, validation_data=(X_test, y_test), epochs=100, callbacks=[stop_early])
